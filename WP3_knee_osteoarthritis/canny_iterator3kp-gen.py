...
from PIL import Image #png-kuvien lukemiseen
...
def find_femur_dot(hsob2, itibiadot, irow, icol, central_bone_deriv):
    ...
    sign_change[central_bone_deriv > -10] = 0 #liian matalat derivaatat pois
...
def find_tibia_dot(hsob2, ifemurdot, irow, icol, central_bone_deriv):
    ...
    sign_change[central_bone_deriv < 10] = 0 #liian pienet derivaatat pois
    if sum(sign_change) == 0:
...
def find_joint_space(img, femur, hsob, gdir):
    ...
    # diffmat[:100,:] = 0 #reunojen pitäisi olla valmiiksi kunnossa
    # diffmat[-100:,:] = 0
    # diffmat[:,:100] = 0
    # diffmat[:,-100:] = 0
    ...
    for offset in range(0,-150, -1):
        ...
        if (abs(imiddle - ijsaverage) < 30 and 
            ...
            miss += 1
            if miss > 3 and abs(offset) > 30: #polvi ei voi olla minikapea sentään
                # plt.figure()
                # plt.plot(central_bone_deriv)
                # plt.figure()
                # plt.imshow(hsob+femurdot*100+tibiadot*100+middledot*100)
                # plt.plot(icol, irow)
                # print("abs imiddle - ijsaverage", abs(imiddle - ijsaverage))
                # print("abs(jsddiff / jsddaverage)", abs(jsddiff / jsddaverage))
                # print("abs(femur_edge_dir - tibia_edge_dir) > 0.5*math.pi", abs(femur_edge_dir - tibia_edge_dir) > 0.5*math.pi)
                # print("jsderiv_diff", jsderiv_diff)
                # print("jsddiff", jsddiff)
                break
...
def find_knee_area(img_orig): #dicomin sijaan suoraan kuvamatriisi
    # alustetaan kuva 
    # img = init_xray_image(dicom_file) #dicomin sijaan suoraan kuvamatriisi

    img = gaussian_filter(img_orig, 4) #
    img = img/img.max()*100 #kuvan arvoavaruus
    # img_small = rescale(img, 0.5) #skaalataan puoleen 1/4:n sijaan
    # print("imgmax", np.max(img))
    ...
    # femur, ilist, jlist = find_femur(img, dpos, dneg) #skipataan
    # luodaan regressiota varten korvaava binäärikuva, jossa keskimmäinen srk
    # on 1 ja loput 0
    femur = np.zeros(img.shape) 
    femur[:,int(femur.shape[1]/2)] = 1 
    ...
    ileft = np.min(np.nonzero(middledot)[1])*1 #ei tarvi skaalausta
    iright = np.max(np.nonzero(middledot)[1])*1 #ei tarvi skaalausta
    imid = int(np.mean(np.nonzero(middledot)[0]))*1 #ei tarvi skaalausta
    halfwidth = int(0.3*(iright-ileft))
    # img, _ = scale(dicom_file, 0.15) #skaalausta ei tehdä
    # img = img[:,100:int(img.shape[1]*0.5)-100] #rajausta ei tehdä
    # plt.figure()
    # plt.imshow(img)
    femur_big = femurdot #rescale(femurdot, 2) #ei tarvi skaalausta
    tibia_big = tibiadot #rescale(tibiadot, 2) #ei tarvi skaalausta
    knee_coord = [imid, halfwidth, ileft, iright]
    
    knee_area = img_orig[imid-halfwidth : imid+halfwidth , ileft : iright]
...
def initialize_joint_space(img, femur_edge):
    ...
    #paksunnetaan  binäärilaajennuksella
    # s = np.array([[1],[0],[1]])
    # s = np.ones((3,3))
...
def initialize_tibia_line(img, femur_edge, params):
    ...
    clow = params["clow"]
    cs = params["smoothing"]
    # print("initialize_tibia_line:{}".format(params))
    # haetaan cannyn kanssa yhteinen viiva
    cimg = canny(img, sigma = cs, low_threshold=clow, high_threshold=chigh)
    init_area = femur_edge * cimg
    init_area[init_area != 0 ] = 1
...
def find_eminentia_edge(path):
    ...
    for fid in fpath:
        ...
        init_area = binary_dilation(init_area, structure=s, iterations=4)
        b = np.zeros(init_area.shape)
        
        b[find_branches(joined_tibia_line) > 3] = 1
        # b =  cv2.cornerHarris((joined_tibia_line*255).astype('uint8'),2,3,0.04)
        
        # corners = cv2.goodFeaturesToTrack((joined_tibia_line*255).astype('uint8'), 10, 0.01, 10)
        ...
        plt.imsave("generoitujen_tuloksia/kl01/"+imgid+".png", drawimg)  
...
if __name__ == '__main__':
    find_eminentia_edge("oai/generoituja/kl01/")
