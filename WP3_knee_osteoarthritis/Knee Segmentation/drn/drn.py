...
class DRN(nn.Module):
    ...
    def __init__(self, block, layers, n_classes=2, n_channels=1,
        ...
        if arch == 'C':
            self.conv1 = nn.Conv2d(n_channels, channels[0], kernel_size=7, stride=1,
            ...
            self.layer0 = nn.Sequential(
                nn.Conv2d(n_channels, channels[0], kernel_size=7, stride=1, padding=3,
                          bias=False),
        ...
        if n_classes > 0:
            ...
            self.fc = nn.Conv2d(self.out_dim, n_classes, kernel_size=1,
...
class DRNSeg(nn.Module):
    def __init__(self, model_name="drn_d_105", n_classes=2, n_channels=1, pretrained_model=None, pretrained=False, use_torch_up=False):
        self.n_channels=n_channels
        self.n_classes=n_classes
        super(DRNSeg, self).__init__()
        model = eval(model_name)(
            pretrained=pretrained, n_classes=n_classes,n_channels=n_channels)
    ...
    def forward(self, x):
        ...
        x = self.base(x)
        x = self.seg(x)
        up = nn.UpsamplingBilinear2d(size=(w,h))
        y=up(x)
        return y
