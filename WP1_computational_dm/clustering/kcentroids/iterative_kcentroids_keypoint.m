function [centers, labels] = iterative_kcentroids_keypoint(X, varargin)
% Description: 
% Perform K-centroids clustering using key points in initialization.  
%
% Function calls:
% [centers, labels] = iterative_kcentroids2(X)
% [centers, labels] = iterative_kcentroids2(X, maxClusters)
% [centers, labels] = iterative_kcentroids2(X, maxClusters, clustMethod)
% [centers, labels] = iterative_kcentroids2(X, maxClusters, clustMethod, distance)
% [centers, labels] = iterative_kcentroids2(X, maxClusters, clustMethod, distance, replicates)
% [centers, labels] = iterative_kcentroids2(X, maxClusters, clustMethod, distance, replicates, useprevCent)
% [centers, labels] = iterative_kcentroids2(X, maxClusters, clustMethod, distance, replicates, useprevCent, initcrit)
% [centers, labels] = iterative_kcentroids2(X, maxClusters, clustMethod, distance, replicates, useprevCent, initcrit, pipelined)
% [centers, labels] = iterative_kcentroids2(X, maxClusters, clustMethod, distance, replicates, useprevCent, initcrit, pipelined, showProgression)
%
% Inputs:
%                  X - Input data set
...
for k = 2:maxClusters
    ...
    kp1 = keypointsComp(X, distance, k, uint32(1.50*maxClusters));
    kp2 = keypointsComp2(X, distance, k, uint32(1.50*maxClusters));
    [~, ~, sumd1] = kcentroids(X, k, 1, distance, 'kmeans++', X(kp1,:));
    [~, ~, sumd2] = kcentroids(X, k, 1, distance, 'kmeans++', X(kp2,:));      
    [~, I] = sort([sumd1; sumd2]);
    kp = [kp1, kp2];        
    kp = kp(:,I(1));      
    [L, C, ~] = kcentroids(X, k, 1, distance, 'kmeans++', X(kp,:));
    centers{k-1} = C;
    labels{k-1} = L;
end
fprintf('Done! \n');
